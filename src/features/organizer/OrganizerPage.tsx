...
function StartGroupPlay({ snapshot, resetGeneration }: { snapshot: TournamentSnapshot; resetGeneration: number }) {
  ...
  return (
    <section className="rounded-card bg-navy p-6 text-white shadow-final">
      <h3 className="text-sm font-semibold">{messages.organizer.groupPlay.heading}</h3>
      <p className="mt-1.5 text-[0.6875rem] text-navy-soft">{messages.organizer.groupPlay.description}</p>
      <Button className="mt-4 bg-white text-navy hover:bg-navy-soft" disabled={!ready || mutation.isPending} onClick={() => setConfirmOpen(true)}>
        <Flag /> {messages.organizer.groupPlay.review}
      </Button>
      {mutation.isError ? <p className="mt-3 text-sm text-white" role="alert">{errorMessage(mutation.error)}</p> : null}
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{messages.organizer.groupPlay.title}</AlertDialogTitle>
            <AlertDialogDescription>{messages.organizer.groupPlay.body}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{messages.common.cancel}</AlertDialogCancel>
            <AlertDialogAction disabled={mutation.isPending} onClick={() => mutation.mutate()}>
              {mutation.isPending ? messages.common.saving : messages.organizer.groupPlay.confirm}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
...
export function OrganizerPage({ snapshot, resetGeneration, onStartScoring }: { snapshot: TournamentSnapshot; resetGeneration: number; onStartScoring: () => void }) {
  ...
  // Lineups stay editable until a match of their fixture starts.
  ...
  return (
    <section className="view-enter space-y-6">
      ...
      {inSetup && snapshot.fixtures.length > 0 ? <StartGroupPlay snapshot={snapshot} resetGeneration={resetGeneration} /> : null}
