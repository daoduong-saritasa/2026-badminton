...
- [x] Run `npm run typecheck` project-wide. Exited 0 on 2026-09-16.
- [x] Run `npm run test:related -- <changed files>` using paths from the real phase diff. The 24-file Phase 4 diff selected 2 test files and 14 tests on 2026-09-16; all passed and the command exited 0. No database-backed test depends on this phase's files, so nothing was skipped.
...
- [x] Inspect home-screen standalone viewport behavior in `src/App.css`, `src/index.css`, and `src/features/scoring/ScoreTracker.tsx`; correct any safe-area or reachable-control issues against the shared responsive requirements. Inspected 2026-09-16, no change: the viewport meta omits `viewport-fit=cover`, so the browser keeps content inside the safe area in standalone mode (the `env(safe-area-inset-*)` paddings resolve to 0); the manifest's default status bar does not overlay content; `100dvh`/`100svh` sizing holds. Enabling `cover` was rejected because ScoreTracker's `fixed right-4 top-4` court selector and the 1rem-margin centered dialogs would then sit under the notch and home indicator.
- [x] Translate English copy Phase 4 missed: the load-error retry button in `src/App.tsx`, the empty playing-now notice in `src/features/tournament/TournamentPage.tsx`, the schedule Start button, court options, and tie-order confirmation title in `src/features/organizer/OrganizerPage.tsx`, and the court options in `src/features/scoring/ScoreTracker.tsx`; add the matching entries to `src/i18n/vi.ts` (amended 2026-09-17).
...
- [x] Re-run the complete phase gate after the translation fix: `npm run typecheck` exited 0; `npm run test:related` over the eight source files of the updated diff selected 1 test file and 3 tests, all passed, none skipped, exit 0 (amended 2026-09-17).
...
- [~] Re-run the spec gate after the translation fix on 2026-09-17: `npm run build` exited 0; `npm test` exited 1 with the same environment block — 11 files and 121 tests passed, 4 integration files failed at setup and 44 scenarios were skipped (amended 2026-09-17).
